...
in
{
  ...
  config = mkIf cfg.enable {
    nix-homebrew.trust.casks =
      optional (cfg.windowManagers.enable && cfg.aerospace.enable) "nikitabobko/tap/aerospace"
      ++ optional (cfg.windowManagers.enable && cfg.sikarugir.enable) "sikarugir-app/sikarugir/sikarugir"
      ++ cfg.extraTrustedCasks;

    homebrew = {
      taps =
        optional (cfg.windowManagers.enable && cfg.aerospace.enable) "nikitabobko/tap"
      ...
      casks =
        ...
        ++ optional (cfg.design.enable && cfg.cursorcerer.enable) "cursorcerer"
        ...
        ++ optional (cfg.windowManagers.enable && cfg.sikarugir.enable) "sikarugir"
        ++ cfg.extraCasks;
    };
  };
}
